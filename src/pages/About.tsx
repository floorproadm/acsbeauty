...
export default function About() {
  ...
  const mapLinks = [
    { name: "Google Maps", url: `https://www.google.com/maps/search/?api=1&query=${encodedAddress}`, icon: "🗺️" },
    { name: "Apple Maps", url: `https://maps.apple.com/?q=${encodedAddress}`, icon: "🍎" },
    { name: "Waze", url: `https://waze.com/ul?q=${encodedAddress}`, icon: "🚗" },
  ];
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="center" className="w-48">
                {mapLinks.map((link) => (<DropdownMenuItem key={link.name} asChild><a href={link.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 cursor-pointer"><span>{link.icon}</span><span>{link.name}</span></a></DropdownMenuItem>))}
              </DropdownMenuContent>
            </DropdownMenu>
          </motion.div>
        </div>
