...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Rescheduling booking ${booking_id} to ${new_start_time}`);
    ...
    // Check if new time slot is available using FreeBusy
    ...
    // Filter out the current booking's event from busy blocks
    const conflictingBlocks = busyBlocks.filter((block: any) => {
      // If the busy block is the current booking, ignore it
      return true; // For now, we'll update the event directly
    });
    ...
    if (booking.google_calendar_event_id) {
      ...
      if (getEventResponse.ok) {
        ...
        // Get service name for event title
        ...
        if (newEventResponse.ok) {
          ...
          // Update the booking with new event ID
          await supabase
            .from('bookings')
            .update({ google_calendar_event_id: newEvent.id })
            .eq('id', booking_id);
        }
      }
    }
